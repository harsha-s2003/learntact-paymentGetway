  <footer class="footer py-5 text-center text-white bg-dark mt-5">
      <div class="container">
          <div class="row">
              <div class="col-md-12 mb-4">
                  <h4 class="mb-3">Useful Links</h4>
                  <div class="row justify-content-center">
                      <div class="col-md-3 col-6 mb-2">
                          <a href="term-condition.html" class="text-white text-decoration-none">Terms & Conditions</a>
                      </div>
                      <div class="col-md-3 col-6 mb-2">
                          <a href="privacy-policy.html" class="text-white text-decoration-none">Privacy Policy</a>
                      </div>
                      <div class="col-md-3 col-6 mb-2">
                          <a href="cancellation-refund-policy.html" class="text-white text-decoration-none">Cancellation
                              & Refund Policy</a>
                      </div>
                  </div>
              </div>
          </div>

          <hr class="bg-white my-4">

          <div class="row">
              <div class="col-md-12">
                  <h5 class="mb-3">Learntact</h5>
                  <p class="mb-2">Nagpur, Maharashtra 440025</p>
                  <a href="mailto:info@learntact.in" class="text-white text-decoration-none">
                      Email: info@learntact.in
                  </a>
              </div>
          </div>

          <div class="row mt-4">
              <div class="col-md-12">
                  <p class="mb-0 small">&copy; 2025 Learntact. All rights reserved.</p>
              </div>
          </div>
      </div>
  </footer>

  <script src="<?= base_url();?>assets/node_modules/bootstrap/dist/js/bootstrap.bundle.js"></script>
  <script src="<?= base_url();?>assets/js/script.js"></script>
  <script type="text/javascript" src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.7.1/jquery.min.js"></script>
  </body>

  </html>